...
권한/CSRF/tenant·transaction·migration 변화는 main에 전달한다. 계정·DB 비밀을 출력하지 않으며 테스트에 운영 DB를 사용하지 않는다. 테스트 역할은 배정된 테스트만 수정하고 deep-reviewer/critic/security는 검토만 수행한다.
